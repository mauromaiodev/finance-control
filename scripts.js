...
const addTransactionIntoDom = ({
  ...
  if (amount > 0 && IsInvestment != true && IsNotCount != true)
    var CSSClass = "plus";
  if (amount < 0 && IsInvestment != true && IsNotCount != true)
    var CSSClass = "minus";
...
function getInvests(transactionsIsInvestment, transactionsAmounts) {
  ...
  return Math.abs(
    investiment.reduce(
      (accumulator, investiment) => accumulator + investiment,
      0
    )
  ).toFixed(2);
}
...
function getNotCount(transactionsIsNotCount, transactionsAmounts) {
  ...
  return arrayJustTotal
    .reduce((accumulator, arrayJustTotal) => accumulator + arrayJustTotal, 0)
    .toFixed(2);
}
...
const getIncomes = (
  transactionsIsInvestment,
  transactionsIsNotCount,
  transactionsAmounts
) => {
  let incomeT = [];
  for (let index = 0; index < transactionsIsNotCount.length; index++) {
    if (
      transactionsIsInvestment[index] != true &&
      transactionsIsNotCount[index] != true
    ) {
...
const updateBalanceValues = () => {
  const transactionsIsInvestment = transactions.map(
    ({ IsInvestment }) => IsInvestment
  );
  const transactionsIsNotCount = transactions.map(
